...
public class EasSyncService extends AbstractSyncService {
    ...
    /**
     * Responds to a fetch request.  The FetchMessageRequest is basically our
     * wrapper for the Fetch service call
     *
     * Request:
     ...
     * @throws IOException
     */
    protected void fetchMessageRequest(FetchMessageRequest req, Double protocolVersion) throws IOException {
        ...
        s.end().end().end().done();
        EasResponse resp = sendHttpClientPost("ItemOperations", new ByteArrayEntity(s.toByteArray()),
                COMMAND_TIMEOUT);
        try {
            int status = resp.getStatus();
            if (status == HttpStatus.SC_OK) {
                if (!resp.isEmpty()) {
                    ...
                    try {
                        if (parser.getStatusCode() == FetchMessageParser.STATUS_CODE_SUCCESS) {
                            ...
                            ExchangeService.callback().loadMessageStatus(msg.mId, EmailServiceStatus.SUCCESS, 100);
                        } else {
                            ExchangeService.callback().loadMessageStatus(msg.mId, EmailServiceStatus.CONNECTION_ERROR, 100);
                        }
